...
def load_config(path: Path) -> dict[str, Any]:
    if not path.exists():
        raise FileNotFoundError(f"Missing M14 configuration: {path}")
...
def ensure_control_table(
    ...
    table = bigquery.Table(
        f"{project_id}.{control_dataset}.m14_unit_economics_control_result",
        schema=[
    ...
    *,
    ...
    insert_errors = client.insert_rows_json(
        (
            ...
            "m14_unit_economics_control_result"
        ),
        control_rows,
    )
    if insert_errors:
        raise RuntimeError(
            f"Could not write M14 control results: {insert_errors}"
        )

    failed = [row for row in control_rows if row["status"] != "PASS"]
    if failed:
        raise RuntimeError(f"M14 controls failed: {failed}")

    summary = unit_economics_summary(
        client,
        project_id,
        datasets["mart"],
        location,
    ...
    manifest_path = (
        ...
        / "data"
        / "generated"
        / "m14_unit_economics_manifest.json"
    )
    ...
    if run_errors:
        raise RuntimeError(f"Could not write M14 pipeline log: {run_errors}")
